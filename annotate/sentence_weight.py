...
for index in range(len(annotate_here_val)):
    ...
    annotate_here_list.append([LexFound_key,MsgUniSeg_key,NewHSWordInSentence_key])

#Create annotate_here dataframe 
annotate_here_df = pd.DataFrame(annotate_here_list, columns=['LexFound','MsgUniSeg','NewHSWordInSentence'])
...
for index in range(len(annotate_here_two_val)):
    ...
    annotate_here_two_list.append([LexFound_key,MsgUniSeg_key,NewHSWordInSentence_key])
    
#Create annotate_here dataframe 
annotate_here_two_df = pd.DataFrame(annotate_here_two_list, columns=['LexFound','MsgUniSeg','NewHSWordInSentence'])
...
for index in range(len(annotate_here_threeval)):
    ...
    annotate_here_three_list.append([LexFound_key,MsgUniSeg_key,NewHSWordInSentence_key])

#Create annotate_here dataframe 
annotate_here_three_df = pd.DataFrame(annotate_here_three_list, columns=['LexFound','MsgUniSeg','NewHSWordInSentence'])
...
lexicon_weight = weight.weight_calculation_sentence(weighted)

weight_pd =pd.DataFrame(lexicon_weight,columns=weight_cols)
weight_pd =weight_pd.to_csv('sentence-weight.csv')
